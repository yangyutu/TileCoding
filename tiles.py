...
#        points = uniform(low=0,high=2*pi,size=(10000,2))
        ...
        diff = responses - predictions
       # print(diff)
        print(diff.shape)
       # print(responses)
       # print(predictions)
        totalError = norm(diff)
        ...
        cmac = CMAC(8, 0.5, 0.1)
        points = ndarray((100,2),dtype = float)
        responses = ndarray((100,),dtype = float)
        for i in xrange(10):
            for j in xrange(10):
                points[i*10+j,0] = 0.1*i
                points[i*10+j,1] = 0.1*j
                responses[i*10+j] = i*i*0.1 + j*j*0.1
